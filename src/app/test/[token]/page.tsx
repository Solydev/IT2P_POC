...
export default function TestPage() {
  ...
  if (state === 'loading') {
    return (
      <div className="min-h-screen bg-a2p-bg flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-a2p-accent border-t-transparent rounded-full animate-spin mx-auto"></div>
          <p className="mt-4 text-a2p-text-secondary">Chargement du test...</p>
        </div>
      </div>
    )
  }
  ...
  if (state === 'error') {
    return (
      <div className="min-h-screen bg-a2p-bg flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-a2p-surface rounded-lg shadow-lg border border-a2p-sand p-8 text-center">
          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-8 h-8 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h2 className="text-2xl font-serif font-semibold text-a2p-text mb-4">
            Erreur
          </h2>
  ...
  if (state === 'intro' && sessionData) {
    return (
      <div className="min-h-screen bg-a2p-bg">
        {/* Header with timer (coming soon) */}
        <div className="bg-a2p-surface border-b border-a2p-sand">
          ...
          <div className="bg-a2p-surface rounded-lg shadow-lg border border-a2p-sand p-8 md:p-12">
            <div className="text-center mb-8">
              ...
              {sessionData.session.coacheeName && (
                <p className="text-a2p-text-secondary text-lg">
                  Bienvenue, {sessionData.session.coacheeName}
                </p>
              )}
            </div>

            <div className="space-y-6 text-a2p-text leading-relaxed">
              <p className="text-lg">
                {INTRO_TEXT.preambule}
              </p>

              <div className="border-t border-a2p-sand pt-6">
                <h2 className="font-serif font-semibold text-xl mb-3">Instructions</h2>
                <div className="whitespace-pre-line text-a2p-text-secondary">
            ...
            <div className="mt-10 text-center">
              <button
                onClick={handleStart}
                className="px-8 py-3 bg-a2p-accent text-white rounded-lg hover:bg-a2p-accent-hover transition-colors font-medium text-lg shadow-md"
              >
            ...
            {sessionData.session.expiresAt && (
              <p className="mt-6 text-center text-sm text-a2p-text-secondary">
                Ce test expire le{' '}
